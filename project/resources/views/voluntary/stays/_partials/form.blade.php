...
<div class="form-row">
    <div class="form-group floating-addon col-sm-4">
        <label for="client_id">
            @lang('labels.common.voluntary')<span class="text-danger ml-1">*</span>
        </label>
        @if($stay->exists)
            <div class="input-group {{ has_error_class('client_id') }}">
                ...
                <input type="hidden" name="client_id" id="client_id"
                       value="{{ old('client_id', $stay->client_id) }}">

                <input type="text" disabled="" class="form-control" value="{{ $stay->client->name }}">
            </div>
        @else
            <div class="input-group {{ has_error_class('client_id') }}">

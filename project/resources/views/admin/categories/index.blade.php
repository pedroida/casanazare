...
@section('content')
    <data-list
        data-source="{{ route('admin.pagination.categories') }}"
        delete-message="@lang('flash.common.confirmation.destroy')"
        url-create="{{ route('admin.categorias.create') }}"
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-striped table-md table-vcenter mb-0">
                            <thead>
                                @include('admin.categories._partials.head')
                            </thead>
                            <tbody>
                                <tr v-if="emptyResult">
                                ...
                                <template v-else>
                                    <tr v-for="(item, index) in items" :key="index">
                                        @include('admin.categories._partials.body')
                                    </tr>
                                </template>
                            </tbody>
                        </table>
